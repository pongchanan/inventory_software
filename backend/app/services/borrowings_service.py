...
def _fetch_s3_bytes(key: str) -> bytes | None:
    try:
        ...
        print(f"[borrowings] S3 fetch failed for key={key}: {exc}")
        return None
...
def process_close_image_diff(db: Session, session_id: int, current_jpeg: bytes) -> None:
    ...
    if prev_key is None:
        print(f"[borrowings] Session #{session_id}: no previous image — skipping diff")
        return

    prev_bytes = _fetch_s3_bytes(prev_key)
    if prev_bytes is None:
        print(
            f"[borrowings] Session #{session_id}: cannot fetch previous image — skipping diff"
        )
        return
    ...
    for label in all_labels:
        ...
        if not item:
            print(
                f"[borrowings] Label '{label}' has no matching active item — skipping"
            )
            continue
        ...
        if diff > 0:
            ...
            print(
                f"[borrowings] Session #{session_id}: '{label}' taken x{diff} by user #{user_id}"
            )
            ...
            for _ in range(returned_count):
                ...
                if active_borrow:
                    ...
                    print(
                        f"[borrowings] Session #{session_id}: no active borrow for "
                        f"user #{user_id} item '{label}' — skipping return"
                    )
            print(
                f"[borrowings] Session #{session_id}: '{label}' returned x{returned_count} by user #{user_id}"
            )

    db.commit()
    print(f"[borrowings] Session #{session_id}: diff complete")
